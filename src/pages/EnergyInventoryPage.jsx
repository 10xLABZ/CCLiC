...
import { ArrowLeft, Zap, BatteryLow } from "lucide-react";
import { toast } from "sonner";
import ConsumableActionModal from "@/components/shared/ConsumableActionModal";

const ENERGY_CONSUMABLES = [
  { id: "ENERGY_25",  name: "Energy Boost (+25)",  amount: 25,  icon: "⚡" },
  { id: "ENERGY_50",  name: "Energy Boost (+50)",  amount: 50,  icon: "⚡" },
  { id: "ENERGY_75",  name: "Energy Boost (+75)",  amount: 75,  icon: "⚡" },
  { id: "ENERGY_100", name: "Energy Refill (+100)", amount: 100, icon: "⚡" }
];

const SHOP_ITEMS = [
  { id: "ENERGY_25",  name: "Energy Boost +25",  amount: 25,  igcPrice: 3500,  crydPrice: 5,  icon: "⚡" },
  { id: "ENERGY_50",  name: "Energy Boost +50",  amount: 50,  igcPrice: 6500,  crydPrice: 10, icon: "⚡" },
  { id: "ENERGY_75",  name: "Energy Boost +75",  amount: 75,  igcPrice: 9500,  crydPrice: 15, icon: "⚡" },
  { id: "ENERGY_100", name: "Energy Refill +100", amount: 100, igcPrice: 14000, crydPrice: 25, icon: "⚡" }
];

export default function EnergyInventoryPage() {
  const navigate = useNavigate();
  const [playerData, setPlayerData] = useState(() => getPlayerData());
  const [resultModal, setResultModal] = useState(null);
  const [confirmModal, setConfirmModal] = useState(null);   // { item, currency }
  const [successModal, setSuccessModal] = useState(null);   // { item, currency, price }
  ...
  const handleUse = async (item) => {
    ...
    // Apply energy delta SERVER-SIDE FIRST — only deduct inventory on success
    const result = await applyServerReward({ energy_delta: item.amount, reason: `use_${item.id}` });
    if (result) {
      ...
      setPlayerData(prev => ({ ...prev, energy: result.new_energy, lastEnergyTimestamp: result.last_energy_timestamp, consumables: merged }));
      window.dispatchEvent(new Event('player_synced'));
      setResultModal({ item, newValue: result.new_energy, remaining: Math.max(0, ownedQty - 1) });
    } else {
  ...
  const ownedItems    = ENERGY_CONSUMABLES.filter(item => (consumables[item.id] || 0) > 0);
    ...
    <div className="min-h-screen bg-[#060a12] text-white pb-20">
      ...
      <div className="pt-[148px] px-3 py-3">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <h1 className="text-lg font-bold text-yellow-400 flex items-center gap-2">🔋 Energy</h1>
          <div className="w-16" />
        </div>

        {/* Regen Notice */}
        <div className="text-[9px] text-white text-center mb-4">Regens +1 every 3 min • Deducted by jobs, assists &amp; battles</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {hasInventory ? (
              <div className="space-y-2">
                {ENERGY_CONSUMABLES.map((item) => {
                  const qty = consumables[item.id] || 0;
                  if (qty === 0) return null;
                  return (
                    <div key={item.id} className="bg-[#0a0f1a] border border-yellow-900/40 rounded-lg p-2.5 flex flex-col gap-1.5">
                      ...
                      <Button size="sm" onClick={() => handleUse(item)} className="w-full text-xs bg-yellow-600 hover:bg-yellow-500 h-7">
                        Use
                      </Button>
                    </div>
                  );
              ...
              >
                <BatteryLow className="w-7 h-7 text-red-400" />
                <p className="text-[11px] font-bold text-red-400 leading-tight">No Energy Boosts!</p>
                <p className="text-[10px] text-red-300/70 leading-tight">Use the shop to buy energy items.</p>
              </div>
            )}
          </div>
          ...
          {/* RIGHT — Shop */}
          <div>
            <div className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-2">Buy Energy</div>
            <div className="space-y-2">
              {SHOP_ITEMS.map((item) => (
