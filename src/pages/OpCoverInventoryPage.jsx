...
const OP_COVER_CONSUMABLES = [
  { id: "OPCOVER_25",  name: "Cover Booster (+25)",  amount: 25,  icon: "🛡️" },
  { id: "OPCOVER_50",  name: "Cover Booster (+50)",  amount: 50,  icon: "🛡️" },
  { id: "OPCOVER_75",  name: "Cover Booster (+75)",  amount: 75,  icon: "🛡️" },
  { id: "OPCOVER_100", name: "Cover Refill (+100)",   amount: 100, icon: "🛡️" }
];

const SHOP_ITEMS = [
  { id: "OPCOVER_25",  name: "Cover Booster +25",  amount: 25,  igcPrice: 2500,  crydPrice: 5,  icon: "🛡️" },
  { id: "OPCOVER_50",  name: "Cover Booster +50",  amount: 50,  igcPrice: 4500,  crydPrice: 9,  icon: "🛡️" },
  { id: "OPCOVER_75",  name: "Cover Booster +75",  amount: 75,  igcPrice: 6000,  crydPrice: 12, icon: "🛡️" },
  { id: "OPCOVER_100", name: "Cover Refill +100",   amount: 100, igcPrice: 7500,  crydPrice: 15, icon: "🛡️" }
];

export default function OpCoverInventoryPage() {
  ...
  useEffect(() => {
    // Sync regen state from server, then refresh from localStorage
  ...
  const handleUse = async (item) => {
    ...
    // Apply op_cover delta SERVER-SIDE FIRST — only deduct inventory on success
    const result = await applyServerReward({ op_cover_delta: item.amount, reason: `use_${item.id}` });
    if (result) {
      ...
      setPlayerData(prev => ({ ...prev, opCover: result.new_op_cover, lastOpCoverTimestamp: result.last_op_cover_timestamp, consumables: merged }));
      window.dispatchEvent(new Event('player_synced'));
      setResultModal({ item, newValue: result.new_op_cover, remaining: Math.max(0, ownedQty - 1) });
    } else {
  ...
  const ownedItems    = OP_COVER_CONSUMABLES.filter(item => (consumables[item.id] || 0) > 0);
    ...
    <div className="min-h-screen bg-[#060a12] text-white pb-20">
      ...
      <div className="pt-[148px] px-3 py-3">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <h1 className="text-lg font-bold text-orange-400 flex items-center gap-2">🛡️ Op Cover</h1>
          <div className="w-16" />
        </div>

        {/* Regen Notice */}
        <div className="text-[9px] text-white text-center mb-4">Regens +1 every 3 min • Deducted by jobs, tips, assists, etc.</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {hasInventory ? (
              <div className="space-y-2">
                {OP_COVER_CONSUMABLES.map((item) => {
                  const qty = consumables[item.id] || 0;
                  if (qty === 0) return null;
                  return (
                    <div key={item.id} className="bg-[#0a0f1a] border border-orange-900/40 rounded-lg p-2.5 flex flex-col gap-1.5">
                      ...
                      <Button size="sm" onClick={() => handleUse(item)} className="w-full text-xs bg-orange-600 hover:bg-orange-500 h-7">
                        Use
                      </Button>
                    </div>
                  );
              ...
              >
                ...
                <p className="text-[11px] font-bold text-red-400 leading-tight">No Cover Boosters!</p>
                <p className="text-[10px] text-red-300/70 leading-tight">Use the shop to obtain cover items.</p>
              </div>
            )}
          </div>
          ...
          {/* RIGHT — Shop */}
          <div>
            <div className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-2">Buy Cover</div>
            <div className="space-y-2">
              {SHOP_ITEMS.map((item) => (
